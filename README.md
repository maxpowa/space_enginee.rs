...
The generated Rust files in `crate-sys/src/` are produced by the C# codegen tool in `codegen/`.
The tool launches Space Engineers, reflects over its assemblies, and outputs `.rs` files with
serde + protobuf derive macros.
...
files directly into `crate-sys/src/`.
...
2. Run the extractor with `--output` pointing at the sys crate:
   ```
