...
public class ReplicationEvents
{
    ...
    public void GenerateRustIdentityTypes(string rsOutputPath)
    {
        ...
        // Generate mod.rs
        GenerateModFile(identityDir, typesWithEvents, GetUniqueTypeName);
    ...
    {
        var filename = Path.Combine(identityDir, "mod.rs");
        using var writer = new StreamWriter(filename);
        
        writer.WriteLine("//! Stable type and event identities for Space Engineers.");
        writer.WriteLine("//!");
        writer.WriteLine("//! These enums use FNV-1a hashes as stable identifiers that don't change");
        writer.WriteLine("//! between game versions. Use `Version` to convert to/from");
        writer.WriteLine("//! version-specific indices at runtime.");
        writer.WriteLine("//!");
        writer.WriteLine("//! Auto-generated by codegen. Do not edit manually.");
        writer.WriteLine();
        writer.WriteLine("#![allow(non_camel_case_types, clippy::all)]");
        writer.WriteLine();
        writer.WriteLine("mod replicated_types;");
        writer.WriteLine("mod static_events;");
        writer.WriteLine("mod instance_events;");
        writer.WriteLine();
        writer.WriteLine("pub use replicated_types::ReplicatedType;");
        writer.WriteLine();
        writer.WriteLine("// Re-export all static event types, payloads, and visitor");
        writer.WriteLine("pub use static_events::*;");
        writer.WriteLine();
        writer.WriteLine("// Re-export all instance event types and parsers");
        writer.WriteLine("pub use instance_events::*;");
    }
